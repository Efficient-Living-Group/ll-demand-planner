...
function isBomMasterDemandParentForPanel(panelId, rawSku) {
  ...
  if (id === 'cusb-us' || id === 'cusb-ca') return /^V[23]-/.test(sku) && /(?:-SET(?:-|$)|-BDL-|\d+X)/.test(sku);
  if (id === 'cusb-uk') return /^(?:CUSB-|LFSB-)/.test(sku) && /-SET(?:-|$)/.test(sku);
  if (id === 'cusb-au' || id === 'cusb-au-snuggle') return /^CUSB-/.test(sku) && /-SET(?:-|$)/.test(sku);
