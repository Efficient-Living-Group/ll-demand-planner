...
function normalizeWarehousePayload(payload, warehouse, containerNumber, poReference) {
...
function buildContainerJourney({
  ...
  const warehouseData = normalizeWarehousePayload(normalizedWarehousePayload, warehouse, container, poReference);
  ...
  const timeline = [
    ...
    milestone(
      ...
      warehouseData.handover.current
        ? `Cirro reports warehouse processing. Exact handover time is not provided.`
