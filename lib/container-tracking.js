...
function selectLecangsRecords(rows, poReference, containerNumber) {
  ...
  const containerMatches = container
    ...
    : [];
  if (containerMatches.length) return containerMatches;
  return po
...
function buildContainerJourney({ containerNumber, poReference, findTeuPayload, lecangsPayload, sourceState = {} }) {
  ...
  const findTeuMismatchDetail = 'Carrier data appears to belong to a different voyage for this reused container number';
  ...
  const timeline = [
    ...
    milestone(
      ...
      'FindTEU',
      !findTeuVoyageMismatch && findTeu.gateOut?.type === 'actual' ? 'complete' : (!findTeuVoyageMismatch && findTeu.discharge?.type === 'actual' ? 'current' : (findTeuUnavailable ? 'unavailable' : 'pending')),
