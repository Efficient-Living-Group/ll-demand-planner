...
function buildContainerJourney({
  ...
  const downstreamTimestamp = lecangs.unloaded.timestamp
  ...
  const completedVoyageArchived = findTeuVoyageMismatch && lecangs.unloaded.complete;
  ...
  const asnDetail = lecangs.linkedAsnCount
    ...
    : (warehouseUnavailable
      ...
      : `No linked ${warehouseName} ASN found`);
  ...
  const timeline = [
    ...
    milestone(
      ...
      warehouseName,
      lecangs.appointment.complete ? 'complete' : (lecangs.available ? 'pending' : (warehouseUnavailable ? 'unavailable' : 'pending')),
    ...
    milestone(
      ...
      warehouseName,
      lecangs.warehouseArrival.complete ? 'complete' : (lecangs.appointment.complete ? 'current' : (warehouseUnavailable ? 'unavailable' : 'pending')),
      lecangs.warehouseArrival.timestamp,
      asnDetail
    ),
    milestone(
      ...
      warehouseName,
      lecangs.handover.complete ? (lecangs.unloaded.complete ? 'complete' : 'current') : (warehouseUnavailable ? 'unavailable' : 'pending'),
      lecangs.handover.timestamp,
    ...
    milestone(
      ...
      'Unloading complete',
      warehouseName,
      lecangs.unloaded.complete ? 'complete' : (warehouseUnavailable ? 'unavailable' : 'pending'),
      lecangs.unloaded.timestamp,
      lecangs.unloaded.complete
        ? `All ${lecangs.activeRecords.length} active linked ASN${lecangs.activeRecords.length === 1 ? '' : 's'} reached unloaded`
        : asnDetail
    )
  ];
  ...
  return {
    ...
    journeyLock: {
      poReference: poReference || '',
      containerNumber: container,
      asnNumbers: [...lecangs.asnNumbers].sort()
    },
    warehouse: {
      ...lecangs,
