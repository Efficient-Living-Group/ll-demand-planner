...
function selectLecangsRecords(rows, poReference, containerNumber) {
  ...
  const poMatches = po
    ? records.filter(record => [record?.poNo, record?.erpNo].some(value => compact(value) === po))
    : [];
  if (poMatches.length) {
    if (!container) return poMatches;
    const exactJourneyMatches = poMatches.filter(record => compact(record?.containerNo) === container);
    if (exactJourneyMatches.length) return exactJourneyMatches;
    const recordsWithoutContainer = poMatches.filter(record => !compact(record?.containerNo));
    return recordsWithoutContainer;
  }
  const containerMatches = container
    ? records.filter(record => compact(record?.containerNo) === container)
    : [];
...
function normalizeCirro(payload, poReference) {
  const rows = asRows(payload);
  const po = compact(poReference);
  const exactPoRows = po
    ? rows.filter(record => compact(record?.reference_no) === po)
    : [];
  const rowsWithoutReference = rows.filter(record => !compact(record?.reference_no));
  const records = exactPoRows.length ? exactPoRows : rowsWithoutReference;
  ...
  const etaValues = scoped.map(record => record?.eta_date).filter(Boolean)
    .sort((a, b) => String(a).localeCompare(String(b)));
  ...
  return {
    ...
    appointment: {
      ...
      timestamp: latestForStatus(7) || etaValues.at(-1) || null
    },
    warehouseArrival: {
      complete: allAtStatus(7),
      timestamp: latestForStatus(7)
    },
    handover: {
      complete: allAtStatus(7),
      timestamp: latestForStatus(7)
...
function buildContainerJourney({
  ...
  const warehouseData = isCirro
    ? normalizeCirro(normalizedWarehousePayload, poReference)
    : normalizeLecangs(normalizedWarehousePayload, container, poReference);
  ...
  const findTeuVoyageMismatch = !!(
    podTime
    && downstreamTime
    && podTime > downstreamTime + (36 * 60 * 60 * 1000)
  );
  const completedVoyageArchived = findTeuVoyageMismatch && warehouseData.unloaded.complete;
  const findTeuUnavailable = findTeuVoyageMismatch || ['error', 'not_configured'].includes(sourceState.findteu);
  const warehouseUnavailable = ['error', 'not_configured', 'not_connected', 'unsupported'].includes(
    sourceState.warehouse || sourceState.lecangs
  );
  const polActual = !findTeuVoyageMismatch && findTeu.polDeparture?.type === 'actual';
  const polTimestamp = findTeuVoyageMismatch ? null : (findTeu.polDeparture?.timestamp || findTeu.pol.etd || null);
  const polPlace = findTeuVoyageMismatch
    ? ''
    : [findTeu.pol.terminal, findTeu.pol.port, findTeu.pol.country].filter(Boolean).join(' · ');
  const podActual = !findTeuVoyageMismatch && findTeu.podArrival?.type === 'actual';
  const podTimestamp = findTeuVoyageMismatch ? null : rawPodTimestamp;
  const podPlace = findTeuVoyageMismatch
    ? ''
    : [findTeu.pod.terminal, findTeu.pod.port, findTeu.pod.country].filter(Boolean).join(' · ');
  const findTeuMismatchDetail = completedVoyageArchived
  ...
  const recordLabel = warehouseData.recordLabel || 'ASN';
  ...
  const timeline = [
    milestone(
      ...
      'Port departure',
      'FindTEU',
      polActual ? 'complete' : (polTimestamp ? 'expected' : (findTeuVoyageMismatch ? mismatchedMilestoneState : (findTeuUnavailable ? 'unavailable' : 'pending'))),
      polTimestamp,
      findTeuVoyageMismatch ? findTeuMismatchDetail : (polPlace || 'Departure port not supplied by carrier')
    ),
    milestone(
      'pod-arrival',
      'Destination port arrival',
      'FindTEU',
      podActual ? 'complete' : (podTimestamp ? 'expected' : (findTeuVoyageMismatch ? mismatchedMilestoneState : (findTeuUnavailable ? 'unavailable' : 'pending'))),
      podTimestamp,
      findTeuVoyageMismatch ? findTeuMismatchDetail : (podPlace || 'Destination port not supplied by carrier')
    ),
    milestone(
      'terminal-release',
      'Terminal release / gate out',
      'FindTEU',
      !findTeuVoyageMismatch && findTeu.gateOut?.type === 'actual' ? 'complete' : (!findTeuVoyageMismatch && findTeu.discharge?.type === 'actual' ? 'current' : (findTeuVoyageMismatch ? mismatchedMilestoneState : (findTeuUnavailable ? 'unavailable' : 'pending'))),
      findTeuVoyageMismatch ? null : (findTeu.gateOut?.timestamp || findTeu.discharge?.timestamp || null),
      findTeuVoyageMismatch ? findTeuMismatchDetail : (findTeu.gateOut?.label || findTeu.discharge?.label || 'Carrier-dependent milestone')
    ),
    milestone(
      'appointment',
      'Warehouse appointment',
      warehouseName,
      warehouseData.appointment.complete ? 'complete' : (warehouseData.appointment.timestamp && warehouseData.available ? 'expected' : (warehouseData.available ? 'pending' : (warehouseUnavailable ? 'unavailable' : 'pending'))),
    ...
    milestone(
      ...
      warehouseName,
      warehouseData.handover.complete ? (warehouseData.unloaded.complete ? 'complete' : 'current') : (warehouseUnavailable ? 'unavailable' : 'pending'),
      warehouseData.handover.timestamp,
  ...
  const nextExpected = timeline.find((item, index) => item.state === 'expected' && index > latestCompleteIndex);
  const current = warehouseData.unloaded.complete
    ? 'Unloading complete'
    : (explicitCurrent?.label || nextExpected?.label || latestComplete?.label || 'Tracking active');
  ...
  return {
    ...
    pol: findTeuVoyageMismatch ? {} : findTeu.pol,
    pod: findTeuVoyageMismatch ? {} : findTeu.pod,
    containerType: findTeuVoyageMismatch ? '' : (findTeu.container?.type || ''),
    carrierScac: findTeuVoyageMismatch ? '' : (findTeuPayload?.data?.scac || findTeuPayload?.scac || ''),
    findTeuVoyageMismatch,
    completedVoyageArchived,
    journeyLock: {
      poReference: poReference || '',
      containerNumber: container,
      asnNumbers: isCirro ? [] : [...warehouseData.asnNumbers].sort(),
    ...
    ...(isCirro ? { cirro: warehouseData } : { lecangs: warehouseData }),
    timeline
  };
}
