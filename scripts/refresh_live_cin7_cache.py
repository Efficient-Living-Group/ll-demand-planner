...
def fetch_shopify_velocity(store_key: str, store: dict[str, str], api_version: str) -> dict[str, Any]:
    ...
    us_units_30d = 0.0
    assigned_us_units_30d = 0.0
    unmapped_us_units_30d = 0.0
    ...
    for page in range(1, 31):
        ...
        for order in orders:
            ...
            for line in order.get("line_items") or []:
                ...
                if c == "US" and dt >= now_30d:
                    us_units_30d += qty
                    if branch_id:
                        assigned_us_units_30d += qty
                    else:
                        unmapped_us_units_30d += qty
    ...
    exact_30d_conservation = abs(us_units_30d - assigned_us_units_30d - unmapped_us_units_30d) < 1e-9
    for branch_id in ("60701", "63764"):
        ...
        branch_velocity["_reconciled"] = exact_30d_conservation and unmapped_us_units_30d == 0
        branch_velocity["_unmappedUsUnits"] = unmapped_us_units
        branch_velocity["_us30dUnits"] = us_units_30d
        branch_velocity["_assignedUs30dUnits"] = assigned_us_units_30d
        branch_velocity["_unmappedUs30dUnits"] = unmapped_us_units_30d
...
def build_purchase_orders(pos_raw: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for po in pos_raw:
        ...
        pos.append({
            ...
            "freightDescription": po.get("freightDescription") or "",
            "surcharge": po.get("surcharge") or 0,
            "surchargeDescription": po.get("surchargeDescription") or "",
            "modifiedCOGSDate": po.get("modifiedCOGSDate"),
            "accountingAttributes": po.get("accountingAttributes") or {},
