...
function sumBranchMaps(views, field) {
  const result = {};
  for (const view of views) {
    for (const [sku, qty] of Object.entries(view?.[field] || {})) {
...
function deficitMap(shopify) {
  return Object.fromEntries(
    Object.entries(shopify || {})
      .map(([sku, qty]) => [sku, Math.max(-Number(qty || 0), 0)])
      .filter(([, qty]) => qty > 0)
  );
}
...
async function verifyPanel(panelId) {
  ...
  assert.deepStrictEqual(
    comparableMap(sumBranchMaps(branchViews, 'cin7')),
    comparableMap(data.cin7),
    `${panelId}: All SOH must equal the sum of warehouse SOH`
  );
  assert.deepStrictEqual(
    comparableMap(sumBranchMaps(branchViews.map(view => ({ shopify: deficitMap(view.shopify) })), 'shopify')),
    comparableMap(deficitMap(data.shopify)),
    `${panelId}: All net component deficit must equal the sum of warehouse component deficits`
  );
  if (branchViews.every(view => view.warehouseMetrics?.demandReconciled === true)) {
    ...
    const sellableSkus = new Set([
      ...Object.keys(data.sellableVelocity || {}),
      ...branchViews.flatMap(view => Object.keys(view.sellableVelocity || {}))
    ]);
    for (const sku of sellableSkus) {
      const total = Number(data.sellableVelocity?.[sku] || 0);
      const branchTotal = branchViews.reduce((sum, view) => sum + Number(view.sellableVelocity?.[sku] || 0), 0);
      assert(
        Math.abs(total - branchTotal) <= 1e-9,
        `${panelId}/${sku}: All sellable velocity ${total} must equal branch sellable velocity ${branchTotal}`
      );
    }
    for (const view of branchViews) {
      const source = view.warehouseMetrics?.demandSourceConservation;
      if (!source?.exactUnitMetadata) continue;
      assert(
        Math.abs(Number(source.us30dUnits || 0) - Number(source.assignedUs30dUnits || 0) - Number(source.unmappedUs30dUnits || 0)) <= 1e-9,
        `${panelId}: exact 30-day units must be conserved before velocity rounding`
      );
      assert.strictEqual(Number(source.unmappedUs30dUnits || 0), 0, `${panelId}: no 30-day US units may be unresolved`);
    }
  }
  ...
  return {
    ...
    netComponentDeficit: options.map((option, index) => Object.values(deficitMap(branchViews[index].shopify)).reduce((sum, qty) => sum + Number(qty || 0), 0)),
